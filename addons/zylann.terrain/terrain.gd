...
func _ready():
	...
	for i in range (0, _data.size()):
		d.append(Util.clone_grid(_data[i]))
	_data = d
	
	_data.resize(5)
	for i in range (1, _data.size()):
		var colorchannel = []
		colorchannel = Util.clone_grid(_data[i])
		_data[i] = colorchannel
...
func _on_terrain_size_changed():
	...
	if is_inside_tree():
		...
		var indexlength = _data.size()
		for i in range(0, indexlength):
			Util.resize_grid(_data[i], terrain_size+1, terrain_size+1, 0)
			...
			if cx >= 0 and cy >= 0 and cx < _chunks_x and cy < _chunks_y:
				...
				if mark_for_undo:
					...
					if not _undo_chunks.has(chunk):
						var data = extract_chunk_data(cx, cy, 0)
						_undo_chunks[chunk] = data


func extract_chunk_data(cx, cy, layer):
	var x0 = cx * CHUNK_SIZE
	var y0 = cy * CHUNK_SIZE
	var cell_data = Util.grid_extract_area_safe_crop(_data[layer], x0, y0, CHUNK_SIZE, CHUNK_SIZE)
	var d = {
		"cx": cx,
		"cy": cy,
		"data": cell_data,
...
func apply_chunks_data(chunks_data):
	for cdata in chunks_data:
		...
		Util.grid_paste(cdata.data, _data[cdata.layer], x0, y0)
...
func pop_undo_redo_data():
	...
	for k in _undo_chunks:
		...
		var redo = extract_chunk_data(undo.cx, undo.cy, 0)
...
func update_chunk(chunk):
	...
	var mesh = Mesher.make_heightmap(_data[0], _normals, _data[1], _data[2], _data[3], _data[4], x0, y0, w, h, smooth_shading, quad_adaptation)
